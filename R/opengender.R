...
og_dict_normalize <- function(x, min_count_default=1, keys_only = FALSE) {
  ...
  if (dict_domain == "gender") {
    ...
    v_adds <- c("n")
  } else if (dict_domain=="classification") {
    v_match <- "key"
    v_cat <- "category"
    v_add_matches <- c("country","year")
    v_adds <- c("n")
  } else if (dict_domain=="identifier") {
    v_match <- "name"
    v_cat <- "id"
    v_add_matches <- c("country","year")
  ...
  for (i in colnames(data_norm)) {
    cf <- paste0("og_clean_",i)
    if (exists(cf)) {
      data_norm %<>% dplyr::mutate( {{i}} := do.call(cf,list(x=!!as.symbol(i))))
    }
  }

  # fill in supplementary key columns
  ...
  if ( "n" %in% v_all ) {
    ...
    data_norm %<>% dplyr::mutate( {{v_cat}} := as.factor( eval(as.symbol(v_cat)) ))
    ...
    # inject record with full range of classifications to ensure full marginals
    # in output
    ...
    margins_norm <-
      purrr::map( margins_sets , function(x) {
        data_norm %>%
          dplyr::mutate(dplyr::across(dplyr::all_of(x), ~ NA)) %>%
          dplyr::group_by(dplyr::across(!n)) %>%
          dplyr::summarize(n = sum(n), .groups="drop")
      }) %>%
      purrr::list_rbind()
    ...
    data_norm %<>%
      dplyr::bind_rows(margins_norm) %>%
      dplyr::distinct()

    # pivot from aggregates to proportions
    data_norm %<>%
      dplyr::group_by(dplyr::across(c(v_match,v_add_matches))) %>%
      dplyr::rename(n_=n) %>%
      dplyr::mutate(n=sum(n_)) %>%
      dplyr::ungroup() %>%
      dplyr::mutate(p=n_/n) %>%
      dplyr::select(!n_) %>%
...
og_dict_process_iso3166 <- function(x) {
  ...
  comment(raw.df) <- "iso3166 country"
  raw.df
}
...
#' @param year_adjust year adjustments
#'
#' @return tibble with imputed class as probability
#' @export
#'
#' @importFrom dplyr select
#' @importFrom dplyr rename
...
add_category_predictions <- function(x,
  ...
  #NOTE: assumes dicts are comprised of {{primary_dict_key}}, year, country, n, pr_* for each category
  ...
  cmp <- col_map[which(col_map!="")]
  cmp <- cmp[intersect(names(cmp), names(all_input_keys))]
  cmp_nm <- names(cmp)

  primary_dict_key <- colnames(dicts.tbl)[[1]]
  dict_keys <- c(primary_dict_key, "country", "year")

  if (!names(all_input_keys)[[1]] %in% names(cmp)) {
    stop("must supply a input key")
  }

  if (!all(names(col_map) %in% names(all_input_keys))) {
    warning("additional columns in col_map ignored")
  }
  ...
  v_na <- setdiff(setdiff(names(all_input_keys),"input_key"), colnames(x_norm.df))
  if (length(v_na) > 0) {
    x_norm.df %<>%
      dplyr::bind_rows(tibble::tibble(!!!v_na, .rows = 0, .name_repair = ~ v_na))
  }
  ...
  if (normalize_key) {
    unmatched_keys.df <-
       x_norm.df %>%
         ...
         dplyr::anti_join(dicts.tbl %>% select( {{primary_dict_key}} ),
                       by=c( input_key_norm = {{primary_dict_key}} ))

    unmatched_keys.df %<>%
      dplyr::mutate(input_key_norm = og_normalize_key(input_key))

    x_norm.df %<>%
      dplyr::rows_update(unmatched_keys.df, by = "input_key" )
  ...
  if (fuzzy_match) {
    ...
    #Sdg.tbl <- dicts.tbl %>% dplyr::select({{primary_dict_key}})
    ...
    fuzzy_match.df %<>%
      dplyr::rename(input_key_fuzzy={{primary_dict_key}}) %>%
      dplyr::group_by(input_key_norm) %>%
      dplyr::slice_min(order_by = input_fuzzy_dist, n = 1) %>%
      dplyr::slice_head(n = 1)  %>%
      dplyr::ungroup()

    fuzzy_match.df %<>%
      select(input_key, input_key_norm = input_key_fuzzy, input_fuzzy_dist )
  ...
  if(country_adjust && ("input_country" %in% names(cmp))) {
    unmatched_keys.df <-
      ...
      dplyr::anti_join(dicts.tbl %>% select( {{primary_dict_key}} , country ),
                       by=c( input_key_norm = primary_dict_key ,
   ...
   # join to unmatched
    match_cur.df <-
      dplyr::left_join(unmatched.df,
              ...
              by = dplyr::join_by(
                input_key_norm = primary_dict_key,
                input_country_norm = "country",
  ...
  #             stringr::str_replace(cmp_nm,"(input_)(.*)","\\2"))

  cmp_rename <- setNames(cmp_nm, cmp)

  match_cum.df %<>%
    dplyr::mutate( {{og_class}} := eval(as.symbol(pr_first)) ) %>%
  ...
  rejoined.df <-
    x %>%
    dplyr::left_join(match_cum.df,  by= unname(cmp)) %>%
    as_tibble()
